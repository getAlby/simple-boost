...
`;var gt=function(t,i,s,e){for(var o,n=arguments.length,r=n<3?i:null===e?e=Object.getOwnPropertyDescriptor(i,s):e,h=t.length-1;h>=0;h--)(o=t[h])&&(r=(n<3?o(r):n>3?o(i,s,r):o(i,s))||r);return n>3&&r&&Object.defineProperty(i,s,r),r};let mt=class extends Q{constructor(){super(),this.address="",this.amount=100,this.currency="sats",this.method="lnurl",this.customKey=null,this.customValue=null,this.noConfetti=!1,this.theme="default",this.isLoading=!1,this.jsConfetti=new vt,0===this.classList.length&&this.classList.add("default"),this.addEventListener("click",this.clickHandler)}get formattedAmount(){if("sats"===this.currency)return`${this.amount} sats`;const t=navigator.language;return new Intl.NumberFormat(t,{style:"currency",currency:this.currency}).format(this.amount)}async getAmountInSats(){return"sats"===this.currency?Promise.resolve(this.amount):fetch(`https://getalby.com/api/rates/${this.currency}`).then((t=>t.json())).then((t=>{const i=t[this.currency.toUpperCase()].rate_float;return this.amount/i*1e8}))}isWeblnSupported(){return!!window.webln}isKeysendSupported(){return this.isWeblnSupported()&&!!window.webln.keysend}async clickHandler(t){if(t.preventDefault(),this.isLoading)return;let i;this.isLoading=!0;try{i=await this.getAmountInSats()}catch(t){return console.error(t),void alert("Could not fetch invoice")}if("lnurl"===this.method){const t=await((t,i)=>{let s=t;if(t.match(/@/)){const[i,e]=t.split("@");s=`https://${e}/.well-known/lnurlp/${i}`}return fetch(s).then((t=>{if(t.ok)return t.json();throw new Error("Network response was not OK")})).then((t=>{const s=new URL(t.callback),e=1e3*i;return s.searchParams.set("amount",e.toString()),fetch(s.toString()).then((t=>{if(t.ok)return t.json();throw new Error("Invoice creation failed")}))}))})(this.address,i),s=t.pr;if(this.isWeblnSupported())try{await window.webln.enable();let i=await window.webln.sendPayment(s);i.preimage&&(this.noConfetti||this.jsConfetti.addConfetti(),this.dispatchEvent(new CustomEvent("success",{detail:{pr:t.pr,preimage:i.preimage}})))}catch(t){this.isLoading=!1}else(t=>{const i=`lightning:${t}`,s=document.createElement("a");s.href=i,s.target="_blank",document.body.appendChild(s),s.click(),document.body.removeChild(s)})(s)}else if("keysend"===this.method)if(this.isKeysendSupported()){let t={};this.customKey&&this.customValue&&(t[this.customKey]=this.customValue);try{let s=await window.webln.keysend({destination:this.address,amount:i,customRecords:t});s.preimage&&(this.noConfetti||this.jsConfetti.addConfetti(),this.dispatchEvent(new CustomEvent("success",{detail:{preimage:s.preimage}})))}catch(t){console.error(t)}}else alert("A Lightning wallet with WebLN support is required. Please install Alby (https://getalby.com)");else alert("Invalid method");this.isLoading=!1}render(){return P`
    ...
    `}};mt.styles=bt,gt([it({type:String})],mt.prototype,"address",void 0),gt([it({type:Number})],mt.prototype,"amount",void 0),gt([it({type:String})],mt.prototype,"currency",void 0),gt([it({type:String})],mt.prototype,"method",void 0),gt([it({type:String,attribute:"custom-key"})],mt.prototype,"customKey",void 0),gt([it({type:String,attribute:"custom-value"})],mt.prototype,"customValue",void 0),gt([it({type:Boolean,attribute:"no-confetti"})],mt.prototype,"noConfetti",void 0),gt([it({type:String})],mt.prototype,"theme",void 0),gt([it({type:Boolean,attribute:!1})],mt.prototype,"isLoading",void 0),mt=gt([(t=>i=>"function"==typeof i?((t,i)=>(customElements.define(t,i),i))(t,i):((t,i)=>{const{kind:s,elements:e}=i;return{kind:s,elements:e,finisher(i){customElements.define(t,i)}}})(t,i))("simple-boost")],mt);export{mt as SimpleBoost};
